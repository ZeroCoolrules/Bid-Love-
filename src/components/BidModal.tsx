...
const BidModal: React.FC<BidModalProps> = ({ listing, datee, currentBalance, onClose, onSubmitBid }) => {
  ...
  const handleSubmit = async () => {
    if (bidAmount < listing.token_ask) {
      setError(`Minimum bid is ${listing.token_ask} Dream Coin`);
      return;
    }
    if (bidAmount > currentBalance) {
      setError('Insufficient Dream Coin balance');
      return;
    }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
        <div className="bg-gradient-to-br from-[#1a0f2e] to-[#0d0618] rounded-3xl p-8 max-w-md w-full border border-white/10 text-center">
          ...
          <p className="text-white/60 mb-4">
            Your bid of <span className="text-[#FFB800] font-bold">{bidAmount} Dream Coin</span> has been submitted to {datee.name}.
          </p>
      ...
      <div className="bg-gradient-to-br from-[#1a0f2e] to-[#0d0618] rounded-3xl max-w-lg w-full border border-white/10 overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          <div className="relative">
            ...
            />
            <span className="absolute right-4 top-1/2 -translate-y-1/2 text-white/40">Dream Coin</span>
          </div>

          <div className="flex justify-between mt-2 text-sm">
            <span className="text-white/50">Minimum: {listing.token_ask} Dream Coin</span>
          ...
          >
            {isSubmitting ? (
              ...
              `Place Bid - ${bidAmount} Dream Coin`
            )}
          </button>

          <p className="text-center text-white/40 text-xs mt-4">
            Dream Coin will be held in escrow until the bid is accepted or rejected
          </p>
        </div>
      </div>
    </div>
