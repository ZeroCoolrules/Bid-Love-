    ...
    try {
      ...
      if (paymentIntent && paymentIntent.status === 'succeeded') {
        ...
        if (confirmError) {
          ...
          setError('Payment successful but failed to add Dream Coin. Please contact support.');
        } else {
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-white/60 mb-4">
          {selectedPackage.tokens + selectedPackage.bonus} Dream Coin has been added to your wallet
        </p>
      ...
      <div className="bg-white/5 rounded-xl p-4 border border-white/10">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-white/70">
            <span>{selectedPackage.tokens} Dream Coin</span>
            <span>${selectedPackage.price.toFixed(2)}</span>
          </div>
          {selectedPackage.bonus > 0 && (
            <div className="flex justify-between text-green-400">
              <span>Bonus Dream Coin</span>
              <span>+{selectedPackage.bonus}</span>
            </div>
          )}
          <div className="border-t border-white/10 pt-2 mt-2">
            <div className="flex justify-between text-white font-bold">
              <span>Total Dream Coin</span>
        ...
        <div className="sticky top-0 z-10 bg-gradient-to-br from-[#1a0a2e] to-[#1a0a2e] p-6 border-b border-white/10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white">Buy Dream Coin</h2>
                <p className="text-white/60 text-sm">Current Balance: {currentBalance.toLocaleString()} Dream Coin</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-6">
          ...
          {!clientSecret ? (
            <>
              {/* Package Selection */}
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-white mb-4">Select a Dream Coin Package</h3>
                  ...
                  {TOKEN_PACKAGES.map((pkg) => (
                    ...
                    >
                      ...
                      <div className="text-white/50 text-sm mt-1">
                        ${(pkg.price / (pkg.tokens + pkg.bonus) * 100).toFixed(1)}¢ per Dream Coin
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div className="bg-white/5 rounded-xl p-5 border border-white/10">
                <h4 className="text-white font-semibold mb-3">Why Buy Dream Coin?</h4>
                <ul className="space-y-2 text-white/70 text-sm">
                  ...
                  <li className="flex items-start gap-2">
                    <CheckIcon size={16} className="text-green-400 mt-0.5 flex-shrink-0" />
                    <span>Earn Dream Coin back from successful dates</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckIcon size={16} className="text-green-400 mt-0.5 flex-shrink-0" />
                    <span>Cash out unused Dream Coin anytime</span>
                  </li>
                </ul>
              </div>
            </>
